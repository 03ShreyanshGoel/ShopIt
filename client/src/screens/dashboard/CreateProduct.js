...
const CreateProduct = () => {
  ...
  const imageHandle = async (e) => { 
    ...
    setState({ ...state, [name]: file }); // Keep the original file in state for preview
    ...
    reader.onloadend = () => {
      setPreview((prevState) => ({
        ...prevState,
        [name]: reader.result, // Update the preview image
      }));
    };
    ...
    formData.append("upload_preset", process.env.REACT_APP_CLOUDINARY_UPLOAD_PRESET); // Your Cloudinary preset
    formData.append("cloud_name", process.env.REACT_APP_CLOUDINARY_CLOUD_NAME); // Your Cloudinary cloud name
    ...
    try {
      ...
      if (data.secure_url) {
        // Successfully uploaded the image, update the state with Cloudinary URL
        setState((prevState) => ({
          ...prevState,
          [name]: data.secure_url,
        }));
        console.log(`${name} uploaded successfully:`, data.secure_url);
      } else {
        toast.error(`Failed to upload ${name}: ${data.message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error(`Error uploading ${name}:`, error);
      toast.error(`Error uploading ${name}: ${error.message || 'Unknown error'}`);
    }
  };
